...
def chart(dayline, stock_name):
    ...
    for i in range(lenth):
        _ohlc = stock_ohlc(o=dayline.iloc[i, 1],
                           h=dayline.iloc[i, 2],
                           l=dayline.iloc[i, 3],
                           c=dayline.iloc[i, 4],
                           t=dayline.iloc[i, 0],
    ...
    for i in range(lenth):
        if i < 10:
            _factor = factor(avrp=100*_ohlc.d[i]/_ohlc.v[i],
                             arip=(_ohlc.h[i]+_ohlc.l[i])/2,
                             mav=dayline.iloc[i, 4],
                             box=(dayline.iloc[i, 4] - dayline.iloc[0: i + 1, 3].min()) /
                             (dayline.iloc[0: i + 1, 2].max() - dayline.iloc[0: i + 1, 3].min()))
        else:
            _factor = factor(avrp=100*_ohlc.d[i]/_ohlc.v[i],
                             arip=(_ohlc.h[i]+_ohlc.l[i])/2,
                             mav=dayline.iloc[i, 4],
                             box=(dayline.iloc[i, 4] - dayline.iloc[i - 9: i + 1, 3].min()) /
                             (dayline.iloc[i - 9: i + 1, 2].max() - dayline.iloc[i - 9: i + 1, 3].min()))
    del dayline
